...
class SolakonModbusHub {
    ...
    connect() {
        ...
        return new Promise((resolve, reject) => {
            ...
            const onError = err => {
                ...
                this._connected = false;
                this._connecting = false;
                this.log.warn(`Modbus connection error: ${err.message}`);
                reject(err);
            };
            ...
            const onConnect = () => {
                ...
                this._connecting = false;
                this.log.info(`Modbus TCP connected: ${this.host}:${this.port} (Unit ID: ${this.unitId})`);
                // Test-Lesen des ersten Registers
                this.client
                    .readHoldingRegisters(30000, 1)
                    .then(() => resolve())
                    .catch(err => {
                        this.log.warn(`Test read failed: ${err.message || err}`);
            ...
            const onClose = () => {
                this._connected = false;
                this._connecting = false;
                this.log.debug('Modbus socket closed');
            };
            ...
            this.socket.on('error', err => {
                this._connected = false;
                this.log.debug(`Socket error (after connect): ${err.message}`);
            });
    ...
    /**
     * Trennt die Verbindung. Versucht zunächst einen sauberen FIN-Abschluss
     * (socket.end()), damit das Gerät seinen Verbindungs-Slot zeitnah freigibt
     * (viele eingebettete Modbus-TCP-Geräte erlauben nur eine gleichzeitige
     * Verbindung); erzwingt spätestens nach 300ms einen harten destroy(),
     * falls das Gerät nicht rechtzeitig reagiert. Löst nie ab (rejected nie).
     *
     * @returns {Promise<void>}
     */
    disconnect() {
    ...
    async readHoldingRegisters(address, count) {
        ...
        try {
            ...
            throw new Error(`Reading register ${address} (x${count}) failed: ${err.message || err}`);
        }
    }
    ...
    async writeSingleRegister(address, value) {
        ...
        try {
            ...
            throw new Error(`Writing to register ${address} failed: ${err.message || err}`);
        }
    }
    ...
    async readAllRegisters() {
        ...
        for (const [key, config] of Object.entries(REGISTERS)) {
            try {
                ...
                this.log.debug(`Register '${key}' (${config.address}) failed: ${err.message}`);
            }
        }
    ...
    async writeNamedRegister(regName, value) {
        ...
        if (!config.rw) {
            throw new Error(`Register '${regName}' is not writable`);
        }
        ...
        if (config.count === 1) {
            ...
            throw new Error(`Writing to register '${regName}' with count=${config.count} not supported`);
        }
