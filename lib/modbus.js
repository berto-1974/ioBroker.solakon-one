...
class SolakonModbusHub {
  ...
  connect() {
    ...
    return new Promise((resolve, reject) => {
      ...
      const onConnect = () => {
        ...
        this._connecting = false;
        this.log.info(
          `Modbus TCP verbunden: ${this.host}:${this.port} (Unit ID: ${this.unitId})`,
        );
      ...
      setTimeout(() => {
        if (this._connecting) {
          ...
          reject(
            new Error(
              `Verbindungs-Timeout nach 10s (${this.host}:${this.port})`,
            ),
          );
        }
      }, 10000);
  ...
  async readHoldingRegisters(address, count) {
    ...
    try {
      ...
      throw new Error(
        `Lesen von Register ${address} (x${count}) fehlgeschlagen: ${err.message || err}`,
      );
    }
  }
  ...
  async writeSingleRegister(address, value) {
    ...
    try {
      ...
      throw new Error(
        `Schreiben auf Register ${address} fehlgeschlagen: ${err.message || err}`,
      );
    }
  }
  ...
  async readAllRegisters() {
    ...
    for (const [key, config] of Object.entries(REGISTERS)) {
      try {
        const rawRegs = await this.readHoldingRegisters(
          config.address,
          config.count,
        );
  ...
  async writeNamedRegister(regName, value) {
    ...
    if (config.count === 1) {
      // 16-Bit Register
      const regVal =
        rawValue < 0 ? (rawValue + 0x10000) & 0xffff : rawValue & 0xffff;
      ...
      throw new Error(
        `Schreiben auf Register '${regName}' mit count=${config.count} nicht unterstützt`,
      );
    }
