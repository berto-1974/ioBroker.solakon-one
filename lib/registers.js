 ...
 */
const REMOTE_CONTROL_MODES_I18N = {
    0: {
        en: 'Off',
        de: 'Aus',
        ru: 'Выкл',
...
/**
 * Löst eine i18n-States-Map ({ key: { en, de, ... } }) für eine Sprache
 * zu einer reinen Record<key, string>-Map auf, wie sie ioBroker als
 * common.states erwartet. Fällt pro Eintrag auf Englisch zurück.
 *
 * @param {{[key: string]: {[lang: string]: string}}} i18nStates - i18n-Quellwerte je States-Key
 * @param {string} lang - gewünschte Sprache (ioBroker-Sprachcode, z. B. "de")
 * @returns {{[key: string]: string}} States-Map mit reinen Strings
 */
function resolveStates(i18nStates, lang) {
    const result = {};
    for (const key of Object.keys(i18nStates)) {
        const entry = i18nStates[key];
        result[key] = entry[lang] || entry.en;
    }
    return result;
}
