...
class SolakonOneAdapter extends utils.Adapter {
  ...
  async onReady() {
    ...
    if (!host) {
      this.log.error(
        "Keine IP-Adresse konfiguriert. Bitte im Admin-Panel einstellen.",
      );
      return;
    }

    this.log.info(
      `Verbinde mit Solakon ONE: ${host}:${port || 502} (Unit ID: ${slaveId || 1})`,
    );
  ...
  async onStateChange(id, state) {
    ...
    try {
      ...
      this.log.error(
        `Schreiben von '${ctrlDef.writeReg}' fehlgeschlagen: ${err.message}`,
      );
    }
  }
